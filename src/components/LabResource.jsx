  ...
  return (
    <div className="w-full border bg-white border-zinc-300 p-[16px] flex flex-col rounded-lg">
      <img
        src={image}
        alt=""
        className="w-[72px] h-[72px] object-cover object-center mb-[8px] rounded-md"
      />
      <div className="flex items-center justify-between mb-[4px]">
        ...
        <span
          className={
            status === "available"
              ? "bg-green-200 px-3 py-1.5 rounded-full text-green-800"
              : "bg-red-200 px-3 py-1.5 rounded-full text-red-800"
          }
        >
          {status}
        </span>
      ...
      <Link className="primary-btn !w-full" to={`/library/${id}/lend`}>
        Request Now
      </Link>
    </div>
  );
